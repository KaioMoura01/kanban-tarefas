...
Nunca usar valores arbitrários `[var(--color-*)]` quando o token já existe no
`@theme` — usar o utilitário gerado (`bg-surface`, `text-content`,
`border-border-subtle` etc.). Valores arbitrários desse tipo disparam o warning
`suggestCanonicalClasses` e são proibidos. Cores fora do `@theme` também devem
ser adicionadas ao `@theme` antes de uso, nunca inline via `[...]`.
