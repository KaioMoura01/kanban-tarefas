...
<div class="pointer-events-none fixed right-4 bottom-4 z-50 flex flex-col items-end gap-2">
	{#each notificationsModule.list as toast (toast.id)}
		...
		<div
			class="bg-surface-elevated pointer-events-auto flex w-[min(24rem,calc(100vw-2rem))] items-center gap-3 rounded-xl border-l-4 px-4 py-3 shadow-lg {tones[
				toast.tone
			]}"
			role="alert"
		>
			<Icon class="text-content size-5 shrink-0" />
			<p class="paragraph-md text-content flex-1">{toast.message}</p>
			<button
				type="button"
